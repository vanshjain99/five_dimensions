      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 lg:h-20">
          <Link to="/" className="flex items-center gap-4">
            ...
            >
              <span className="text-white font-bold text-l tracking-tight">5D</span>
            </div>
            <span
              className="font-bold text-xl lg:text-3xl tracking-wide transition-colors duration-200"
              style={{ fontFamily: FONT_SERIF, color: showSolidBar ? COLORS.navy : COLORS.navy }}
            >
              Five Dimensions
            </span>
          </Link>
          ...
          <div className="hidden lg:flex items-center gap-8">
            {NAV_LINKS.map(({ label, href }) => (
              <a
                ...
                style={{ color: showSolidBar ? COLORS.navy : 'rgba(255,255,255,0.88)' }}
              >
                {label}
              </a>
            ))}
